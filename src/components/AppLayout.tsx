...
export function AppLayout() {
  ...
  return (
    <div
      className="grid min-h-screen max-[900px]:grid-cols-1"
      style={{ gridTemplateColumns: collapsed ? "64px 1fr" : "260px 1fr" }}
    >
      {/* ── Sidebar ── */}
      <aside className={`flex flex-col gap-2 border-r border-[rgba(153,176,255,0.12)] bg-[rgba(6,11,22,0.65)] [backdrop-filter:blur(18px)] ${collapsed ? "px-2 py-4" : "p-5"}`}>
        ...
        <div className={`flex items-start ${collapsed ? "justify-center" : "justify-between"} mb-4`}>
          {!collapsed && (
            <div className="px-4">
              ...
              {appVersion && (
                <span className="text-xs text-[#9aa8c7]">v{appVersion}</span>
              )}
            </div>
          )}
          <button
        ...
        <nav className="flex flex-col gap-1">
          <NavLink
            ...
            className={({ isActive }) =>
              collapsed
                ? isActive ? iconNavLinkActive : iconNavLink
